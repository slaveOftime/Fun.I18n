...
type TypeProvider(config: TypeProviderConfig) as this =
    ...
    let generator =
        ProvidedTypeDefinition(asm, ns, "I18nProvider", Some typeof<obj>, isErased = true)
            ...
            if Regex.IsMatch(arg, "^https?://") then
                let result =
                    ...
                    async {
                        ...
                        if status <> 200 then failwithf "URL %s returned %i status code" arg status
                        return res
                    }
                ...
                let content =
                    if arg.StartsWith("{") || arg.StartsWith("[") then
                        ...
                        try
                            let filePath = System.IO.Path.GetFullPath arg
                        ...
                        with
                            | e -> failwith $"Read file failed: {e.Message} {e.StackTrace}"
